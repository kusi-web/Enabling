...
class assets_report(models.Model):
    ...
    def get_header(self, options):
        ...
        return [
            [
                {'name': ''},
                {'name': _('Asset'), 'colspan': 8},
                {'name': _(' Cost'), 'colspan': 5},
            ...
            [
                ...
                {'name': _('Purchase Price'), 'class': 'number'},
                {'name': _('Total Accumulated Depreciation'), 'class': 'number'},
                {'name': _('Total Revaluation'), 'class': 'number'},
                ...
                {'name': _('Current Period Revaluations'), 'class': 'number'},
                {'name': _('Current Period Additions'), 'class': 'number'},
                {'name': _('Current Period Disposals'), 'class': 'number'},
                {'name': end_date, 'class': 'number'},
                {'name': _('Accumulated Depreciation at'), 'class': 'number'},  # Depreciation
                {'name': _('Current Period Depreciation'), 'class': 'number'},
                {'name': _('Disposals'), 'class': 'number'},
                {'name': _('Accumulated Depreciation at'), 'class': 'number'},
                {'name': _('Net Book Value'), 'class': 'number'},  # Gross
            ],
            [
                {'name': ''},  # Description
                {'name': _(''), 'class': 'text-center'},  # Characteristics
                {'name': _(''), 'class': 'number'},
                {'name': _(''), 'class': 'number'},
                {'name': _(''), 'class': 'number'},
                {'name': _(''), 'class': 'number'},
                {'name': _(''), 'class': 'number'},
                {'name': _(''), 'class': 'number', 'title': _('In percent.<br>For a linear method, the depreciation rate is computed per year.<br>For a declining method, it is the declining factor'), 'data-toggle': 'tooltip'},
                {'name': _(''), 'class': 'number'},
                {'name': '', 'class': 'number'},  # Assets
                {'name': _(''), 'class': 'number'},
                {'name': _(''), 'class': 'number'},
                {'name': _(''), 'class': 'number'},
                {'name': '', 'class': 'number'},
                {'name': _('' + start_date), 'class': 'number'},  # Depreciation
                {'name': _(''), 'class': 'number'},
                {'name': _(''), 'class': 'number'},
                {'name': _('' + end_date), 'class': 'number'},
                {'name': _(''), 'class': 'number'},  # Gross
            ],
        ]
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for company_id, company_asset_lines in groupby(asset_lines, key=lambda x: x['company_id']):
            ...
            for al in parent_lines:
                ...
                asset_depreciation_rate = ('{:.2f} %').format(float(al['asset_depreciation_percentage'] or '0.0'))
                ...
                # depreciation_opening = company_currency.round(al['depreciated_start'] * al_rate) - company_currency.round(al['depreciation'] * al_rate) + company_currency.round((al['import_depreciated'] or 0.00) * al_rate)
                ...
                asset_total_depreciation = company_currency.round(al['asset_total_depreciation'] * al_rate)
                if depreciation_opening == 0.00 and depreciation_closing == 0.00:
                    if al['asset_date'] and al['asset_date'] >= fields.Date.to_date(options['date']['date_from']) and al['asset_date'] and al['asset_date'] <= fields.Date.to_date(options['date']['date_to']):    
                        depreciation_closing += company_currency.round((al['import_depreciated'] or 0.00) * al_rate)
                    if al['asset_date'] and al['asset_date'] < fields.Date.to_date(options['date']['date_from']):
                        depreciation_opening +=  company_currency.round((al['import_depreciated'] or 0.00) * al_rate)
                else:
                    if al['asset_date'] and al['asset_date'] > fields.Date.to_date(options['date']['date_from']):
                        depreciation_opening -=  company_currency.round((al['import_depreciated'] or 0.00) * al_rate)
                
                asset_x_purch_price = company_currency.round((al['asset_x_purch_price'] or 0.00) * al_rate)
                asset_x_book_reval = company_currency.round((al['asset_x_book_reval'] or 0.00) * al_rate)
                asset_x_book_reval_current = company_currency.round((al['asset_x_book_reval_current'] or 0.00) * al_rate)
                asset_x_book_reval_before = company_currency.round((al['asset_x_book_reval_before'] or 0.00) * al_rate)
                ...
                # asset_value_residual = - (company_currency.round((al['asset_value_residual'] or 0.00) * al_rate) + company_currency.round((al['import_depreciated'] or 0.00) * al_rate))
                # asset_value_residual = (company_currency.round((al['asset_value_residual'] or 0.00) * al_rate) + company_currency.round((al['import_depreciated'] or 0.00) * al_rate))
                ...
                if al['asset_acquisition_date'] and al['asset_acquisition_date'] >= fields.Date.to_date(options['date']['date_from']) and al['asset_acquisition_date'] <= fields.Date.to_date(options['date']['date_to']):
                    asset_add = asset_x_purch_price

                # if al['asset_value_addition'] or al['asset_value_revaluation']:
                #     asset_opening =  asset_x_purch_price + asset_x_book_reval_before + asset_x_value_addition_before
                asset_opening = asset_x_book_reval_before + asset_x_value_addition_before
                if al['asset_acquisition_date'] and al['asset_acquisition_date'] < fields.Date.to_date(options['date']['date_from']):
                    asset_opening +=  asset_x_purch_price
                ...
                for child in children_lines[al['asset_id']]:
                    ...
                    # depreciation_opening += company_currency.round(child['depreciated_start'] * child_rate) - company_currency.round(child['depreciation'] * child_rate) + company_currency.round(child['import_depreciated'] * child_rate)
                    ...
                    if child['asset_date'] and child['asset_date'] > fields.Date.to_date(options['date']['date_from']):
                        depreciation_opening -=  company_currency.round((child['import_depreciated'] or 0.00) * child_rate)
                    
                    # asset_value_residual -= (company_currency.round(child['asset_value_residual'] * child_rate) + company_currency.round(child['import_depreciated'] * child_rate))
                    # asset_value_residual += (company_currency.round(child['asset_value_residual'] * child_rate) + company_currency.round(child['import_depreciated'] * child_rate))
                    ...
                    # if child['asset_value_addition'] or child['asset_value_revaluation']:
                    #     asset_opening +=  company_currency.round(child['asset_x_purch_price'] * child_rate) + company_currency.round(child['asset_x_book_reval_current'] * child_rate)
                    # else:
                    #     asset_opening += company_currency.round(child['asset_original_value'] * child_rate) if opening else 0.0
                    asset_opening += company_currency.round(child['asset_x_book_reval_before'] * child_rate) + company_currency.round(child['asset_x_value_addition_before'] * child_rate)
                    if child['asset_acquisition_date'] and child['asset_acquisition_date'] < fields.Date.to_date(options['date']['date_from']):
                        asset_opening += company_currency.round(child['asset_x_purch_price'] * child_rate)
                    ...
                    if child['asset_acquisition_date'] and child['asset_acquisition_date'] >= fields.Date.to_date(options['date']['date_from']) and child['asset_acquisition_date'] <= fields.Date.to_date(options['date']['date_to']):
                        child_asset_add = company_currency.round(child['asset_x_purch_price'] * child_rate)
                    ...
                    asset_add += child_asset_add  + company_currency.round((child['asset_x_value_addition_current'] or 0.00) * child_rate)

                if depreciation_closing - depreciation_opening > 0 :
                ...
                if al['asset_disposal_date'] and al['asset_disposal_date'] >= fields.Date.to_date(options['date']['date_from']) and al['asset_disposal_date'] <= fields.Date.to_date(options['date']['date_to']):
                    ...
                    asset_minus = asset_opening + asset_x_book_reval_current + asset_add
                    
                depreciation_closing = depreciation_opening + depreciation_add - depreciation_minus
                asset_closing = asset_opening + asset_x_book_reval_current + asset_add - asset_minus 
                ...
                # depreciation_opening = - depreciation_opening
                # depreciation_add = - depreciation_add
                # depreciation_closing = - depreciation_closing
                if al['asset_state'] == 'close':
                    asset_total_disposals = asset_x_purch_price + asset_x_book_reval + asset_x_value_addition - asset_total_depreciation
                total_net_book_value = asset_x_purch_price + asset_x_book_reval + asset_x_value_addition - asset_total_depreciation - asset_total_disposals

                total = [x + y for x, y in zip(total, [asset_x_purch_price, asset_total_depreciation, asset_x_book_reval, asset_x_value_addition, asset_total_disposals, total_net_book_value,asset_opening, asset_x_book_reval_current, asset_add, asset_minus, asset_closing, depreciation_opening, depreciation_add, depreciation_minus, depreciation_closing, asset_gross])]
                ...
                line = {
                    ...
                    'columns': [
                        ...
                        {'name': self.format_value(asset_x_purch_price), 'no_format_name': asset_x_purch_price},
                        {'name': self.format_value(asset_total_depreciation), 'no_format_name': asset_total_depreciation},
                        {'name': self.format_value(asset_x_book_reval), 'no_format_name': asset_x_book_reval},
                        ...
                        {'name': self.format_value(asset_x_book_reval_current), 'no_format_name': asset_x_book_reval_current},
        ...
        lines.append({
            ...
            'columns': [
                ...
                {'name': ''},
                {'name': self.format_value(total[5])},  # Assets
                {'name': self.format_value(total[6])},
                {'name': self.format_value(total[7])},
                {'name': self.format_value(total[8])},
                {'name': self.format_value(total[9])},
                {'name': self.format_value(total[10])},  # Depreciation
                {'name': self.format_value(total[11])},
                {'name': self.format_value(total[12])},
                {'name': self.format_value(total[13])},
                {'name': self.format_value(total[14])}, 
                {'name': self.format_value(total[15])},# Gross
            ],
            'unfoldable': False,
            'unfolded': False,
        })
    ...
    def _get_assets_lines(self, options):
        ...
        """
        self.flush()
        self.env.cr.execute(revaluation_sql, {'date_to': date_to, 'date_from': date_from, 'company_ids': company_ids})
        revaluation_results = self.env.cr.dictfetchall()
        for result in results:
            result['asset_x_book_reval_current'] = 0.00
            for revaluation_result in revaluation_results:
                if result['asset_id'] == revaluation_result['asset_id']:
                    result['asset_x_book_reval_current'] = revaluation_result['sum']
                    break
        ...
        """
        self.flush()
        self.env.cr.execute(revaluation_before_sql, {'date_from': date_from, 'company_ids': company_ids})
        revaluation_results = self.env.cr.dictfetchall()
        for result in results:
            result['asset_x_book_reval_before'] = 0.00
            for revaluation_result in revaluation_results:
                if result['asset_id'] == revaluation_result['asset_id']:
                    result['asset_x_book_reval_before'] = revaluation_result['sum']
                    break
